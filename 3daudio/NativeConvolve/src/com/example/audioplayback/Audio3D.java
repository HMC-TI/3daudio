...
public class Audio3D {	
	...
	void createNewOutputs() {
		if (chanFlag) { // If we're taking in 2 inputs, we need 4 convolutions
			 ...
			 ************************************************/
			NativeConvolve new_rl = new NativeConvolve(oldInBuf_l, inBuf_l, irfBuf[1]);
			NativeConvolve new_rr = new NativeConvolve(oldInBuf_r, inBuf_r, irfBufR[0]);
			NativeConvolve new_ll = new NativeConvolve(oldInBuf_l, inBuf_l, irfBuf[0]);
			NativeConvolve new_lr = new NativeConvolve(oldInBuf_r, inBuf_r, irfBufR[1]);
			...
			if (cfFlag) {
				...
				for (int i = 0; i < time_samples; i++) {
					...
					// inBuf_l
			NativeConvolve new_ll = new NativeConvolve(oldInBuf_l, inBuf_l, irfBuf[0]);
			NativeConvolve new_lr = new NativeConvolve(oldInBuf_l, inBuf_l, irfBuf[1]);
	...
	void createOldConvolveAndCrossfade() {
		...
		if (chanFlag) {
			 ...
			 * Changed
			 ******************************************/
			NativeConvolve old_rl = new NativeConvolve(oldInBuf_l, inBuf_l, oldIrf[1]);
			NativeConvolve old_rr = new NativeConvolve(oldInBuf_r, inBuf_r, oldIrfR[0]);
			NativeConvolve old_lr = new NativeConvolve(oldInBuf_l, inBuf_l, oldIrf[0]);
			NativeConvolve old_ll = new NativeConvolve(oldInBuf_r, inBuf_r, oldIrfR[1]);
			...
			NativeConvolve old_ll = new NativeConvolve(oldInBuf_l, inBuf_l, oldIrf[0]);
			NativeConvolve old_lr = new NativeConvolve(oldInBuf_l, inBuf_l, oldIrf[1]);
