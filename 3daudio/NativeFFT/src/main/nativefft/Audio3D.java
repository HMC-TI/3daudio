...
public class Audio3D {	
	...
	COMPLEX_DATUM irfBuf;
	/******************************************************
	 * New variables
	 ******************************************************/
	COMPLEX_DATUM irfBufR;
	COMPLEX_DATUM oldIrfR;
	/*******************************************************/
	COMPLEX_DATUM oldIrf;
	...
	void createNewOutputs() {
		if (chanFlag) { // If we're taking in 2 inputs, we need 4 convolutions
			 ...
			 ************************************************/
			AudioFFT new_rl = new AudioFFT(irfBuf.right[0], irfBuf.right[1], inBuf_l);
			AudioFFT new_rr = new AudioFFT(irfBufR.left[0], irfBufR.left[1], inBuf_r);
			AudioFFT new_ll = new AudioFFT(irfBuf.left[0], irfBuf.left[1], inBuf_l);
			AudioFFT new_lr = new AudioFFT(irfBufR.right[0], irfBufR.right[1], inBuf_r);
			...
			if (cfFlag) {
				...
				for (int i = 0; i < time_samples; i++) {
					...
					// inBuf_l
			AudioFFT new_ll = new AudioFFT(irfBuf.left[0], irfBuf.left[1], inBuf_l);
			AudioFFT new_lr = new AudioFFT(irfBuf.right[0], irfBuf.right[1], inBuf_l);
	...
	void createOldConvolveAndCrossfade() {
		...
		if (chanFlag) {
			 ...
			 * Changed
			 ******************************************/
			AudioFFT old_rl = new AudioFFT(oldIrf.right[0], oldIrf.right[1], inBuf_l);
			AudioFFT old_rr = new AudioFFT(oldIrfR.left[0], oldIrfR.left[1], inBuf_r);
			AudioFFT old_lr = new AudioFFT(oldIrf.left[0], oldIrf.left[1], inBuf_l);
			AudioFFT old_ll = new AudioFFT(oldIrfR.right[0], oldIrfR.right[1], inBuf_r);
			...
			AudioFFT old_ll = new AudioFFT(oldIrf.left[0], oldIrf.left[1], inBuf_l);
			AudioFFT old_lr = new AudioFFT(oldIrf.right[0], oldIrf.right[1], inBuf_l);
