...
/* Modified by Allison Card on 2/14/13 to include a complex class
 * and to take the FFT of the HRTF.
 */
...
public class GetIRF {
	...
	public COMPLEX_DATUM[][] irf_data;

	GetIRF () {
		irf_data = new COMPLEX_DATUM[14][72];
		read_irfs();
	...
	{
		int nfaz;
		IRFTransform transformer = new IRFTransform();
		...
		for (int el_index = 0; el_index < N_ELEV; el_index++) 
		{
			...
			{
				//actually load IRFs
				IRF_DATUM irfData = read_irf(el_index, az_index);
				irf_data[el_index][az_index].right = transformer.getComplexResults(irfData.right);
				irf_data[el_index][az_index].left = transformer.getComplexResults(irfData.left);
			}
		}
	}

	//Uses bufferedReader to read in IRF data line by line based on elevation and azimuth
	public IRF_DATUM read_irf(int el_index, int az_index)
	{		
		...
		IRF_DATUM irfData = new IRF_DATUM();
        ...
        {	
            ...
            {	
        		...
        		{
        			irf_data[el_index][az_index] = new COMPLEX_DATUM();
            	...
            	if(linecount >= time_samples)
            	{
            		...
            		irfData.right[linecountmod] = (float) sample;
            	}
            	...
            	{		
            		...
            		irfData.left[linecount] = (float) sample;
            	}
	...
	public COMPLEX_DATUM get_irf(double elev, double azim)
	{
		...
		//System.out.println("Retrieved COMPLEX_DATUM from irf_data");
		...
		returnArray.left = hd.left;
		returnArray.right = hd.right;
